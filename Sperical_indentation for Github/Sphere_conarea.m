...
NUM=1500;                     
infinity=100;                 
area=logspace(log10(0.05),log10(3.5),50)*2;    %dimensionless contact area
...
while(abs(deri2)>=0.00001)
    temp=guess2-0.4*(deri2/(deri2-deri1))*(guess2-guess1);
    guess1=guess2;
    guess2=temp;
...
r = sol.x;
f = sol.y;
inter=linspace(0.01,conarea,300);
...
delta(j)=w0;
fn(j)=force;
a_num(j)=conarea;     %numerical contact area
if w0>=0.33
    break;
end
...
c2=-(-2*k0n*eta-k1n*ln*eta*eta+w0*k1n*ln*rs)/(rs*(k0p*k1n*ln-k0n*k1p*lp));
c4=-(2*k0p*eta+k1p*lp*eta*eta-w0*k1p*lp*rs)/(rs*(k0p*k1n*ln-k0n*k1p*lp));
inter1=linspace(0.01,eta,300);
sphere=-w0+(inter1.*inter1)./(r_indenter);
Fout(j)=abs(32*pi*c2/lp*eta*besselk(1,lp*eta)+32*pi*c4/ln*eta*besselk(1,ln*eta));
Fin(j)=abs(trapz(inter1,-2*pi*vdWW(sphere).*inter1));
end

a_theo(j)=eta;   %theoretical contact area
end
ax = gca;
    ...
    ylabel('$\eta=a/l$','interpreter','latex','fontsize',20);
loglog(delta,a_num,'Color',col(i,:),'LineWidth',1.5);
hold on
delta
a_theo
loglog(delta(1:length(a_theo)),a_theo,'Color',col(i,:),'LineWidth',1.5,'LineStyle','--');
end
conarea
% --------------------------------------------------functions
function vdW=vdWW(r)
